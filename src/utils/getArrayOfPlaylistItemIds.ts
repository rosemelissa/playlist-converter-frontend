...
import { IYoutubeSearchData, IYoutubeSearchResponse, TitleAndImg } from "./interfaces";
...
async function getArrayOfPlaylistItemIds(
  ...
  const titleAndImgArr: TitleAndImg[] = [];
    ...
    try {
        const response: IYoutubeSearchResponse = await axios.get(`https://youtube.googleapis.com/youtube/v3/playlistItems?part=snippet%2CcontentDetails&maxResults=50&playlistId=${playlistId}&key=${YOUR_API_KEY}`);
        ...
        while (nextPageToken) {
            const pageResponse: IYoutubeSearchResponse = await axios.get(`https://youtube.googleapis.com/youtube/v3/playlistItems?part=snippet%2CcontentDetails&maxResults=50&playlistId=${playlistId}&key=${YOUR_API_KEY}&pageToken=${nextPageToken}`);
