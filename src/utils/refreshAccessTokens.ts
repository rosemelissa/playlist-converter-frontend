...
async function refreshAccessToken(): Promise<void> {
    ...
    if (refresh_token && CLIENTID) {
        ...
        const result: ISpotifyAPI = await axios.post("https://accounts.spotify.com/api/token", body, headers);
        ...
        const t = new Date();
        const new_expires_at = t.setSeconds(t.getSeconds() + result.data.expires_in);
