...
import { ISpotifySearchResponse, ISpotifyTrack, IYoutubeAndSpotify } from "../utils/interfaces";
...
function SearchForDifferentTrack({setMode, thisTrack, spotifySearchResults, setSpotifySearchResults, indexOfThisTrack}: SearchForDifferentTrackProps): JSX.Element {
    const [searchTerm, setSearchTerm] = useState<string>("");
    const [searchResults, setSearchResults] = useState<ISpotifyTrack[]|null>(null)
    ...
    const searchForNewTracks = async () => {
        ...
        try{ const searchResults: ISpotifySearchResponse = await axios.get(`https://api.spotify.com/v1/search?q=name:${searchTerm}&type=track&limit=5`, headers);
    ...
    const useThisTrack = () => {
        ...
        const newResults: IYoutubeAndSpotify[] = spotifySearchResults.map((result, i) => {
            if (newTrack) {
    ...
    return (
        <>
            ...
            <button type="button" onClick={searchForNewTracks}>Search</button>
            {searchResults && searchResults.map((thisTrack, i) => 
                <div key={i} onClick={() => {
                    (newTrack && newTrack.id === thisTrack.id) ? setNewTrack(null): setNewTrack(thisTrack)}}>
            ...
            {searchResults && <button type="button" onClick={() => {setMode('display')}}>Cancel</button>}
            {newTrack && 
            <button type="button" onClick={useThisTrack}>Use this track</button>
            }
